<?php if ( ! defined( 'ABSPATH' ) ) exit;
    ...
    $options = get_option('ariafontrealhomes_font_settings');
if(!empty($options['fontname1']) && !empty($options['fontname2']) && !empty($options['fontname3']) ){
        $realhomes_fonts = array($options['fontname1'], $options['fontname2'], $options['fontname3']);
        
    }
    elseif(!empty($options['fontname1']) && !empty($options['fontname2'])){
        $realhomes_fonts = array($options['fontname1'], $options['fontname2']);
       
    }
    elseif(!empty($options['fontname1']) && !empty($options['fontname3'])){
        $realhomes_fonts = array($options['fontname1'], $options['fontname3']);
        
    }
    elseif(!empty($options['fontname2']) && !empty($options['fontname3'])){
        $realhomes_fonts = array($options['fontname2'], $options['fontname3']);
        
    }
    elseif(!empty($options['fontname1'])){
        $realhomes_fonts = array($options['fontname1']);
        
    }
    elseif(!empty($options['fontname2'])){
        $realhomes_fonts = array($options['fontname2']);
        
    }
    elseif(!empty($options['fontname3'])){
        $realhomes_fonts = array($options['fontname3']);
...
<div class="wrap">
    ...
    <form method="post" action="options.php">
        <?php settings_fields('ariafontrealhomes_font_settings'); ?>
        <?php $options = get_option('ariafontrealhomes_font_settings'); ?>

        
        
        <hr/>
        <table class="form-table">
            <h3><?php _e('فونت قالب ریل هوم', 'awp'); ?></h3>
            <p>
                از بخش زیر فونت هایی را که قصد دارید در قالب ریل هوم استفاده کنید، انتخاب بفرمایید.
            </p>
            <p>
            ...
            <tr valign="top">
                ...
                <td>
                    <select name="ariafontrealhomes_font_settings[fontname<?php echo $i; ?>]" id="ariafontrealhomes_font_settings[fontname]">
            ...
            <tr valign="top">
                ...
                <td>
                    <select name="ariafontrealhomes_font_settings[bodyfontname]" id="ariafontrealhomes_font_settings[bodyfontname]">
               <option value="">هیچکدام</option>
               <optgroup label="فونت ها">
               <?php 
               for($i=0;$i<count($realhomes_fonts);$i++){?>
                  <option <?php echo ($options['bodyfontname'] == $realhomes_fonts[$i] ? "selected ":""); ?>value="<?php echo $realhomes_fonts[$i];?>"><?php echo $realhomes_fonts[$i]; ?></option>
               <?php }?>
               </optgroup>
            ...
            <tr valign="top">
                ...
                <td>
                    <select name="ariafontrealhomes_font_settings[hfontname]" id="ariafontrealhomes_font_settings[hfontname]">
               <option value="">هیچکدام</option>
               <optgroup label="فونت ها">
               <?php 
               for($i=0;$i<count($realhomes_fonts);$i++){?>
                  <option <?php echo ($options['hfontname'] == $realhomes_fonts[$i] ? "selected ":""); ?>value="<?php echo $realhomes_fonts[$i];?>"><?php echo $realhomes_fonts[$i]; ?></option>
               <?php }?>
               </optgroup>
            ...
            <tr valign="top">
                ...
                <td>
                    <select name="ariafontrealhomes_font_settings[menufontname]" id="ariafontrealhomes_font_settings[menufontname]">
               <option value="">هیچکدام</option>
               <optgroup label="فونت ها">
               <?php 
               for($i=0;$i<count($realhomes_fonts);$i++){?>
                  <option <?php echo ($options['menufontname'] == $realhomes_fonts[$i] ? "selected ":""); ?>value="<?php echo $realhomes_fonts[$i];?>"><?php echo $realhomes_fonts[$i]; ?></option>
               <?php }?>
               </optgroup>
