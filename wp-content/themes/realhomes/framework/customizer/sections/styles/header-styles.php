...
if ( ! function_exists( 'inspiry_header_styles_customizer' ) ) :
	...
	function inspiry_header_styles_customizer( WP_Customize_Manager $wp_customize ) {
		...
		if ( 'modern' === INSPIRY_DESIGN_VARIATION ) {
			...
			$wp_customize->add_setting( 'inspiry_top_menu_gradient_color_1', array(
				'type'              => 'option',
				'sanitize_callback' => 'sanitize_hex_color',
			) );
			$wp_customize->add_control(
				new WP_Customize_Color_Control(
					$wp_customize,
					'inspiry_top_menu_gradient_color_1',
					array(
						'label'       => esc_html__( 'Header Background Gradient One Color', 'framework' ),
						'section'     => 'inspiry_header_styles',
						'description' => esc_html__( 'Starting gradient color from top (default #000000)', 'framework' ),
					)
				)
			);
			$wp_customize->add_setting( 'inspiry_top_menu_gradient_color_2', array(
				'type'              => 'option',
				'sanitize_callback' => 'sanitize_hex_color',
			) );
			$wp_customize->add_control(
				new WP_Customize_Color_Control(
					$wp_customize,
					'inspiry_top_menu_gradient_color_2',
					array(
						'label'       => esc_html__( 'Header Background Gradient Two Color', 'framework' ),
						'section'     => 'inspiry_header_styles',
						'description' => esc_html__( 'Finishing gradient color to bottom (default #c0c0c0)', 'framework' ),
					)
				)
			);
		}
