...
class RealHomes_WPLM {
	...
	public function menu_content() {
        ...
        <div class="wrap wpmlr_wrap" data-show="<?php echo $data_show ?>">
			...
			<?php else: ?>
               <p>جهت برخورداری از کلیه امکانات قالب لطفا کلید لایسنس خود را ثبت کنید.</p>
                <form action="#" method="post" class="register_version_form">
            ...
            <script>
                jQuery(document).ready(function($) {
                    ...
                    echo var_dump($a)echo var_dump(ajax_url)
     ...
     * If you want add an interrupt in your plugin or theme simply can use this static method: RealHomes_WPLM::is_activated
     * This will return true or false for license status
	 * @return bool
	 */
	public static function is_activated() {
		$private_session = get_option('RealHomes_theme_rgstr');
		$now = json_decode(get_option($private_session));
		if( empty($now) ) {
			return false;
		} elseif($now->action != 1) {
...
add_action('init', 'realhomes_wplm_init');
/**
 * Initialize function for class and hook it to wordpress init action
 */
function realhomes_wplm_init() {
	$settings = [
		'name'          => 'لایسنس قالب',
		'slug'          => 'realhomes-wplm-register',
		'parent_slug'   => 'real_homes', // Read this: https://developer.wordpress.org/reference/functions/add_submenu_page/#parameters
		'text_domain'   => 'your-text-domain',
		'product_token' => '0069cfeb-5ad4-4005-a85a-ad184ae9655e', // Get it from here: https://zhaket.com/dashboard/licenses/
		'option_name'   => 'realhomes_theme_rgstr'
	];
	RealHomes_WPLM::instance($settings);
}
